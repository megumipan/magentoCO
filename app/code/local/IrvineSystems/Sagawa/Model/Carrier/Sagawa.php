...
class IrvineSystems_Sagawa_Model_Carrier_Sagawa extends Mage_Shipping_Model_Carrier_Abstract
{
	// Model Constants
	...
	const TRACK_URL_JAP = 'http://k2k.sagawa-exp.co.jp/p/web/okurijosearch.do?okurijoNo=';
	const TRACK_URL_ENG = 'http://k2k.sagawa-exp.co.jp/p/sagawa/web/okurijosearcheng.jsp';
	...
	protected $_locMethodCodes = array('hikyakumail','hikyakuexpress');
	...
	*/
	public function collectRates(Mage_Shipping_Model_Rate_Request $request)
	{
		...
		// Get Local Shipping Rates if the destination country is Japan
       if($this->_dstCountryId == self::JAPAN){
			// Check if any local shipments is enable
			$_locEnable = false;
			foreach ($this->_locMethodCodes as $_method) {
	...
	*/
	public function shipLocal($cartPrice)
	{
		...
		// Get the Size Class for the current Cart Dimensions
		$SizeClass = $resource->_getSizeClass($orderDimension,$this->_cartWeight);	

		// Get the additional Package label if available
		$packTypeLabel = $this->_getpackageLabel($SizeClass);
		
		// Process each Local Methods
		foreach ($this->_locMethodCodes as $method_code) {

			// Skip the Method if it is not enabled
			if(!Mage::getStoreConfig(self::CODE.'/' .$method_code .'/showmethod')) continue;

			// Chek if the method is mail or express
			if($method_code == 'hikyakumail'){
				// Get the Method Title
				$method_title = Mage::getStoreConfig(self::CODE.'/'.$method_code .'/title');
				
				// Validate if the method can be processed
				if(!$resource->validateMail($method_code,$this->_finalVolumes,$this->_cartWeight)) continue;

				// Get the rate for the current Cart
				$rate = $resource->getMailRate($this->_cartWeight);

				// Do not process if the rate is not available
				if(!$rate || $rate == 0) continue;
			}else{
				// Get the Method Title
				$method_title = Mage::getStoreConfig(self::CODE.'/'.$method_code .'/title').$packTypeLabel;
	...
	/**
	* Get the Shipping Method type of Label
	* (Standard Size || Free Size)
	*
    * @param int $SizeClass = Current Order Size Class
    * @return string
	*/
	protected function _getpackageLabel($SizeClass)
	{
		// Define Label
		$packTypeLabel = '';
		// Set the Package Additional Label according to the size class
		if($SizeClass <= 4){
			$packTypeLabel = Mage::getStoreConfig(self::CODE.'/general/stdlabel');
		}else{
			$packTypeLabel = Mage::getStoreConfig(self::CODE.'/general/freelabel');
		}
		// If we have a result add it in Parathesis
		if(!empty($packTypeLabel)) $packTypeLabel = ' ('.$packTypeLabel.')';
		// Return the Label
	...
	*/
    ...
    public function getAllowedMethods()
    {
		...
		$carMethods = array (
			'hikyakumail'				=> Mage::helper(self::CODE)->__('Hikyaku Mail Shipment'),
			'hikyakuexpress'			=> Mage::helper(self::CODE)->__('Hikyaku Express')
		);
	...
	public function getTracking($trackings)
    {
		...
		// URL pre and post parts
		$trackUrlPre = '';
		$trackUrlPost = '';

		// Process HTML link formatting only for Magento Version before 1.7
        ...
        foreach($trackings as $trackingNum){
			$trackingURL =  $trackUrlPre.self::TRACK_URL_JAP.$trackingNum.$trackUrlPost;
			// Use normal Link if English User
			if ($lang == 'en') $trackingURL = $trackUrlPre.self::TRACK_URL_ENG.$trackUrlPost;
