...
class IrvineSystems_Yamato_Helper_Data extends Mage_Core_Helper_Abstract{
	...
	*/
	...
	/* -------------------------------------------------------- */ 
	/* ------ EXPRESS SHIPPING OPTION HELPER FUNCTIONS -------- */ 
	/* -------------------------------------------------------- */ 

    /**
     * Validate if cooling shipping can be performed
     * 
     * @param $method Current shipping method
     * 
     * @return bool
     */
	public function canExpress($method)
	{       
		
		// Return false if the method is not mailbin
		if ($method != 'mailbin') return false;
		// Return false if the express service is disable
		if (!Mage::getStoreConfig(self::CODE.'/mailbin/showmailexpress')) return false;
		// if by any chance the fee is incorrect return false
		if (Mage::getStoreConfig(self::CODE.'/mailbin/mailexpressfee') < 0) return false;
     ...
     * @return bool
     */
	public function getExpressRate($formatted = false)
	{
		// Get the configured fee
		$basefee = Mage::getStoreConfig(self::CODE.'/mailbin/mailexpressfee');
		// Convert the Rate
		$fee = $this->convertRate($basefee,true);
		// Check if we want the String Formatted Price
		if($formatted) $fee = Mage::helper('core')->currency($fee);
		// Return the fee
		return $fee;
	}
     ...
     * @return bool
     */
	public function canCool($method)
	{       
		// Return false if the method is not taqbin
		if ($method != 'taqbin') return false;
		
		// Check if the Cool Shipments is enable in configuration
		if (!Mage::getStoreConfig(self::CODE.'/taqbin/showcool')) return false;
	...
	/* -------------------------------------------------------- */ 
	/* ----------- CASH ON DELIVERY HELPER FUNCTIONS ---------- */ 
	/* -------------------------------------------------------- */ 
	...
	{       
		...
		$currShipment = Mage::getSingleton('checkout/session')->getQuote()->getShippingAddress()->getShippingMethod();

		// Return false for not compatible shipping methods
		if($currShipment != self::CODE.'_taqbin') return false;
		
		// Check if the Cod is enable in configuration
		if (!Mage::getStoreConfig(self::CODE.'/taqbin/showcod')){
			return false;
		}else{
			// Get shipping resource
			$resource = Mage::getResourceModel(self::CODE.'/local');
			...
			// Return false if the cart amount is invalid
			if(!$resource->validateCod($_subtotal+$_shippingAmount)) return false;
		}
	...
	{       
		...
		if (Mage::getStoreConfig(self::CODE.'/taqbin/internalcod')){
	...
	{       
		...
		if (Mage::getStoreConfig(self::CODE.'/taqbin/internalcod')){
			// Get the Internal Fees
			$tax = $this->getInternalCodTaxFee();
		}else{
			// Get the Cash on deliver Payment fee in base currency and convert it to price list currency
			$tax = $this->convertRate($quote->getBaseCodTaxAmount());
		}
		// return the Fee
		return $tax;
	}
	...
	public function getInternalCodFee($toBase = false)
	{
		...
		if (!Mage::getStoreConfig(self::CODE.'/taqbin/internalcod')) return null;
		...
		$fee += $resource->getCodChargeTax($_subtotal+$_shippingAmount);
	 ...
	 * @return int||float
	 */
	public function getInternalCodTaxFee($toBase = false)
	{
		// Do not process if the internal fee are disabled
		if (!Mage::getStoreConfig(self::CODE.'/taqbin/internalcod')) return null;
