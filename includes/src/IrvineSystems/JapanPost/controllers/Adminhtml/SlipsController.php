...
class IrvineSystems_JapanPost_Adminhtml_SlipsController extends Mage_Adminhtml_Controller_Action
{
    /**
     * Controller Initialization
     * 
     * @return IrvineSystems_JapanPost_Adminhtml_SlipsController
     */
	protected function _initAction()
	{
		//load the Layout
		$this->loadLayout()
			->_setActiveMenu('japanpost/slips')
			->_addBreadcrumb(Mage::helper('japanpost')->__('Shipping Slips'), Mage::helper('japanpost')->__('Shipping Slips'));
	...
	public function indexAction()
	{
		...
		$this->_initAction()
			->_addContent($this->getLayout()->createBlock('japanpost/adminhtml_slips'))
			->renderLayout();
	}
	...
	public function cleanAction()
	{
		...
		if($numOfUpdates > 0){
			Mage::getSingleton('adminhtml/session')
				->addSuccess(Mage::helper('japanpost')
    ...
    public function editAction()
    {
		...
		if ($id) {
            $model->load($id);
            if (! $model->getSlipId()) {
                Mage::getSingleton('adminhtml/session')->addError(Mage::helper('japanpost')->__('This slip no longer exists'));
                $this->_redirect('*/*');
                return;
            }
        }
        ...
        $this->loadLayout();
        $this->_setActiveMenu('japanpost');
        $block = $this->getLayout()->createBlock('japanpost/adminhtml_slips_edit')
            ->setData('action', $this->getUrl('*/japanpost_slips/save'));
		...
		$this->_addContent($block)
			->_addLeft($this->getLayout()->createBlock('japanpost/adminhtml_slips_edit_tabs'))
            ->renderLayout();
    }
	...
	public function saveAction()
	{
		...
		if (!$this->getRequest()->getParams()) {
				Mage::getSingleton('adminhtml/session')->addError(Mage::helper('japanpost')->__('Slip data to be saved were not found. Please refresh the page and try again'));
				$this->_redirect('*/*');
				return;
		}
		...
		if (empty($slipId)){
			Mage::throwException(Mage::helper('japanpost')->__('No slips were specified. Please check the Slip Id'));
               $this->_redirect('*/*/');
			return;
		}
		...
		if ($slipId != $model->getSlipId()) {
			Mage::throwException(Mage::helper('japanpost')->__('Wrong slip specified.'));
               $this->_redirect('*/*/');
			return;
		}
		...
		Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('japanpost')->__('Slip was successfully saved'));
	...
	public function exportCsvAction()
	{
		...
		$fileName = 'JapanPost_Slips.csv';
		// render the content in CSV Format
		$content = $this->getLayout()->createBlock('japanpost/adminhtml_slips_pgrid')->getCsv();
    ...
    {
		// Set name for the file
        $fileName   = 'JapanPost_Slips.xml';
		// render the content in XML Format
        $content    = $this->getLayout()->createBlock('japanpost/adminhtml_slips_pgrid')->getXml();
		// Convert the content format into SJIS-win encoding
        $content = mb_convert_encoding($content, 'SJIS-win', 'auto');
       // Start the Download
        $this->_sendUploadResponse($fileName, $content);
