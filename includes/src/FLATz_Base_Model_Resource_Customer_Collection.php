 ...
 */
class FLATz_Base_Model_Resource_Customer_Collection extends Mage_Customer_Model_Resource_Customer_Collection {

    /**
     * 
     * @return \FLATz_Base_Model_Resource_Customer_Collection
     */
    public function addNameToSelect() {
        ...
        if (isset($fields['prefix'])) {
            $concatenate[] = $adapter->getCheckSql(
                    '{{prefix}} IS NOT NULL AND {{prefix}} != \'\'', 'LTRIM(RTRIM({{prefix}}))', '\'\'');
        }
        $concatenate[] = 'LTRIM(RTRIM({{lastname}}))';
        if (isset($fields['middlename'])) {
            $concatenate[] = $adapter->getCheckSql(
                    '{{middlename}} IS NOT NULL AND {{middlename}} != \'\'', 'LTRIM(RTRIM({{middlename}}))', '\'\'');
        }
