...
class IrvineSystems_JapanPost_Helper_Data extends Mage_Core_Helper_Abstract{
	...
	*/
	...
	{       
		...
		if (!Mage::getStoreConfig(self::CODE.'/yuupack/showcool')) return false;
		...
		// Set the result
		$result = $resource->validateCool($packageSize,$packageWeight);
		// Return the result
		return $result;
	}
	...
	{       
		...
		// Convert the Rate
		$rate = $this->convertRate($rate,true);
	...
	{       
		...
		if($currShipment != self::CODE.'_yuupack') return false;
		
		// Check if the Cod is enable in configuration
		if (!Mage::getStoreConfig(self::CODE.'/yuupack/showcod')) return false;
	...
	public function getCodFee()
	{
		...
		if (Mage::getStoreConfig(self::CODE.'/yuupack/internalcod')){
	...
	public function getInternalCodFee($toBase = false)
	{
		...
		if (!Mage::getStoreConfig(self::CODE.'/yuupack/internalcod')) return null;
		// Get the Config Fee
		$fee = Mage::getStoreConfig(self::CODE.'/yuupack/codfee');
	...
	*/
	...
	{
		// Add any logic for custome delivery times
		return null;
	}
